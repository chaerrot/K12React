  ...
  render() {
    ...
    */
    ...
    if (this.state.mode==='list') { 
      ...
      /*
        부모가 자식 컴포넌트로 데이터를 전달할 때는 props를 사용한다.
        자식이 부모로 데이터를 전달할 때는 Event를 통해 전달한다. 단 이 때
        부모가 내려준 props의 기능(함수)을 사용한다.
      */
      myBoardView={(pnum, pmode) => {
        /*
          2개의 파라미터를 통해 화면의 상태(mode)를 view로 변경하고,
          서버와 통신 시 사용할 일련번호를 받아온다.
        */
        console.log("num", pnum, "mode", pmode);
        // 파라미터를 통해 state를 변경하고 화면을 업데이트한다.
        this.setState({
          mode:pmode, num:pnum
        });
      }}
      ...
      /*
        ...
        일련번호는 state의 num을 변경한다. 이 때 새롭게 렌더링되면서 얻어온 num을
        ViewContents컴포넌트의 props로 사용한다.ㄴ
      */
      ...
      // 폼값을 submit했을 때 서버로 전송하기 위한 함수를 props로 전달
