  ...
  render() {
    ...
    if (this.state.mode === 'welcome') {
      ...
      */
      ...
      // 여기서 처리하면 렌더링이 두 번 되므로 비효율적이다.
    }
    // Navi컴포넌트로 배열로 정의된 contents를 props로 전달한다.
    return (
      <div className="App">
        {/* 부모가 자식에게 함수 기능을 가진 props를 내려준다. 여기서는
          onChangePage라는 props를 정의했다. 해당 함수는 state 중 mode를
        ...
        <Navi data={this.state.contents}
          ...
          <Buttons onChangeMode={(btn_mode) => {
            // 여기서 삭제 처리를 하면 렌더링을 한 번만 해도 되므로 효율적이다.
            if (btn_mode==='delete') {
              // 리액트에서 confirm을 사용할 때는 반드시 window를 붙여야 한다.
