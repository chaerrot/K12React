...
function App() {
  ...
  /*
    useState() 사용 시 전달된 true는 좌측항의 첫 번째 인자로 전달된다.
  ...
  */
  ...
  return (
    <div className="App">
        ...
        {/* 
          이벤트 리스너에 사용한 함수는 useState()의 두 번째 반환값으로
...
/*
클래스형 컴포넌트
  : React.Component를 상속하여 선언한다. 수명주기 함수 중에
...
class ClassComponent extends React.Component {
  ...
  */
  UNSAFE_componentWillMount() {
    /*
      해당 함수를 17버전 이상에서 사용할 때는 UNSAFE_를 붙이지 않으면
      경고메세지가 뜨게 된다.
    */
  ...
  /*
    최초 렌더링 시에는 호출되지 않고, state값이 변경에 의해 재렌더링이 될 때
    호출된다. 해당 함수에서 true가 호출될 때만 render()가 호출된다.
  ...
  */
  ...
  /*
    클래스형 컴포넌트에서는 2개 이상의 state를 필요하면 콤마로
    구분하여 추가하면 된다.
  */
  render() {
    ...
    return (
      <div className="container">
        ...
        {/* 
          해당 버튼을 누를 때마다 난수를 생성하여 state의 number를 변경한다.
...
/*
  함수형 컴포넌트
    : 출력할 내용을 즉시 return하면 된다. 함수 안에 또 다른 함수를 사용할 수
    없으므로 render()함수가 별도로 존재하지 않고, 자기 자신이 render()
    역할을 한다.
*/
function FuncComponent(props) {
  /*
    함수형 컴포넌트에서는 return이 render()의 역할을 하므로 특정 함수를
  ...
  */
  ...
  /*
    해당 컴포넌트가 렌더링된 후 자동으로 호출된다. 해당 함수는 첫 번째 인자로
    반드시 함수가 와야 한다.
    클래스형 컴포넌트는 마운팅 단계와 업데이트 단계에서 사용하는 별도의 수명주기
    함수가 있지만, 함수형 컴포넌트에는 별도로 존재하지 않는다.
  */
  useEffect(function() {
    console.log('#Life3#', 'FuncComponent ==> useEffect');
  });

  // 함수형 컴포넌트는 return이 될 때 렌더링된다.
      ...
      */}
      <input type="button" value="난수생성" onClick={function() {
        ...
        {/* 
          ...
          없으므로, 어떤 형식의 함수를 사용해도 별도의 바인딩이 필요하지 않다.
        */}
