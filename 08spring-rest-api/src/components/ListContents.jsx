...
class ListContents extends Component {
    /*
        ...
        state값이 변경될 때마다 render()함수가 호출되어 화면이 갱신된다.
    */
    state = {
        blists: [],
        pageNum: 1
    ...
    /*
        ...
        첫 렌더링에서는 빈 화면을 먼저 출력하고, 서버와 통신 후 데이터가 있는
        화면을 출력한다.
    */
    componentDidMount() {
        ...
        스프링 서버로 요청한 후 콜백데이터가 있으면 then()절에서 콜백 받아 사용한다.
        */
        fetch('http://localhost:8081/jsonrestapi/restapi/boardList.do?nowPage='+ pageNum) // 페이지 X
        ...
        .then(function(json) {
            ...
            이와 같이 state를 변경할 수 있으나, 이 경우 render()함수가 호출되지
            않으므로 화면이 갱신되지 않는다. 반드시 setState()를 통해 state값을
            변경해야 render()함수가 호출되어 화면이 업데이트된다.
            */
    ...
    render() {
        ...
        // 인덱스르 통해 해당 반복에서 출력할 객체를 저장
        var row = this.state.blists[i];
        // push()를 통해 배열의 마지막 부분에 데이터 삽입
        /*
            tr태그에 key는 리액트가 중복되지 않는 key prop을 요구하므로 삽입된다.
            data-id: event 객체를 통해 dataset.id 속성값으로 지정된 값을 얻어올 수
        ...
        listTr.push(
            ...
            <td align='left'><a href={row.num} data-id={row.num} onClick={
                ...
                /* 
                    ...
                    전달한다. 게시물의 일련번호와 화면의 mode 값이 사용된다.
                */
