...
/*
  ...
  Switch
    : URL과 일치하는 첫 번째 컴포넌트가 발견되면 나머지 컴포넌트는 아예
    렌더링하지 않는 역할을 한다. 즉 최초로 발견되는 컴포넌트 하나만 렌더링한다.
  Link
    : a 태그를 통한 링크를 클릭할 때마다 화면이 새로고침되어 갱신된다. React는
    화면의 깜빡임이 없어야하므로, 링크를 눌렀을 때 reload되지 않도록 처리해준다.
*/
  ...
  for (var i=0; i<contents.length; i++) {
    ...
    liTag.push(
      <li key={contents[i].id}>
        <NavLink to={"/Topics/"+ contents[i].cate}>
          {contents[i].title}
        </NavLink></li>
    );
    /*
...
/*
  useParmas(): 파라미터로 전송된 값을 받아오기 위한 리액트 훅으로
...
function Desc() {
  ...
  // 일치하는 파라미터가 없을 경우 출력할 기본 내용 추가
  let selected_item = {
    title: "Sorry", desc: "Not Found"
  }
  // 배열 내에서 파라미터와 일치하는 값이 있는지 확인한다.
  for (var i=0; i<contents.length; i++) {
  ...
  return (
    ...
    <div className="App">
      ...
      */}
      ...
      <h3>Switch 적용</h3>
      {/* 
        Switch 컴포넌트를 사용하는 경우 첫 번째 Route에 exact를 기술하지 않으면
        '/'가 모든 URL을 포함하는 형태이므로 아래쪽에 정의된 컴포넌트는 렌더링되지
        않는다.
